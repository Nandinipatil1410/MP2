...
### Insight: The Local-Only Bottleneck
Through benchmarking, we identified several critical limitations of purely local processing (on ~4B parameter models) that validate our Hybrid approach:

1. **Instruction Following Decay**: Without high-level planning, small local models often suffer from "Mirroring"—where they simply repeat the user's query back instead of synthesizing an answer.
2. **Context-Saliency Conflict**: Small models struggle to distinguish between instructions and dense academic context, leading to circular reasoning (e.g., looping on specific terms like "social contract").
3. **Absence of Logical Scaffolding**: Local models perform significantly better when given "Logical Scaffolding" (step-by-step reasoning plans) generated by a larger Cloud LLM, rather than being asked to perform complex synthesis in a single zero-shot step.
